//
//  MissTaskDetailsViewController.m
//  ShuwxApp
//
...
@interface MissTaskDetailsViewController ()<UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
@property (nonatomic, strong) UICollectionView *collectionView;
@end

@implementation MissTaskDetailsViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"MissTaskDetailsViewController"];
  
}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:@"MissTaskDetailsViewController"];
}
...
- (void)initView{
   _personNib = [UINib nibWithNibName:@"MissTaskDetailsCollectionViewCell" bundle:nil];
      ...
      [self.passBtn addTarget:self action:@selector(setPassSubmit) forControlEvents:UIControlEventTouchUpInside];
     [self.noPassBtn addTarget:self action:@selector(setNoPassSubmit) forControlEvents:UIControlEventTouchUpInside];
           
        [self getSelectMyJobLineLoseInfoByJobId];
}
-(void)setNoPassSubmit{
    CKAlertViewController *alertVC = [CKAlertViewController alertControllerWithTitle:@"提示" message:@"确定审核不通过？" ];
         ...
         CKAlertAction *sure = [CKAlertAction actionWithTitle:@"确定" handler:^(CKAlertAction *action) {
             NSLog(@"点击了 %@ 按钮",action.title);
             [self getUpdateMyJobLineLoseIsPassInfoByJobId:@"0"];
...
-(void)setPassSubmit{
    CKAlertViewController *alertVC = [CKAlertViewController alertControllerWithTitle:@"提示" message:@"确定审核通过？" ];
         ...
         CKAlertAction *sure = [CKAlertAction actionWithTitle:@"确定" handler:^(CKAlertAction *action) {
             NSLog(@"点击了 %@ 按钮",action.title);
             [self getUpdateMyJobLineLoseIsPassInfoByJobId:@"1"];
...
-(void)getUpdateMyJobLineLoseIsPassInfoByJobId:(NSString *)passint{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString * baseUrl = [defaults objectForKey:@"baseUrl"];
    ...
    NSDictionary *infoDictionary = [[NSBundle mainBundle] infoDictionary];
       ...
       [diction setValue:@(app_Version) forKey:@"version_code"];
    [diction setValue:self.job_id forKey:@"job_id"];
      [diction setValue:passint forKey:@"is_miss_pass"];

 
    NSString * url = [self getPinjieNSString:baseUrl :updateMyJobLineLoseIsPassInfoByJobId];
...
//        }
//        if ([myResult isKindOfClass:[NSDictionary class]]) {
//            [self setUpdateMyJobLineLoseIsPassInfoByJobId:myResult];
//        }
//        if ([myResult isKindOfClass:[NSString class]]) {
//            [self showToastTwo:myResult];
//        }
        ...
        if (myCode == 200) {
             [self setUpdateMyJobLineLoseIsPassInfoByJobId];
        }
...
-(void)setUpdateMyJobLineLoseIsPassInfoByJobId{
    NSString * resultStr = @"操作成功!";
    
    MyEventBus * myEvent = [[MyEventBus alloc] init];
          myEvent.errorId = @"ReviewIngViewController";
             [[QTEventBus shared] dispatch:myEvent];
    
    MyEventBus * myEventEnd = [[MyEventBus alloc] init];
    myEventEnd.errorId = @"ReviewEndViewController";
             [[QTEventBus shared] dispatch:myEventEnd];
