   ...
   [self clearData];
    ...
    [QTSub(self, MyEventBus) next:^(MyEventBus *event) {
        __strong typeof(self) strongSelf = weakSelf;
          ...
          if ([strongSelf getNSStringEqual:myErrorId:@"ReviewIngViewController"]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        static NSString *  ReviewTableCellView = @"CellId";
        //自定义cell类
        ReviewTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ReviewTableCellView];
        if (!cell) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary * dicOne  =   [self.dataSource objectAtIndex:indexPath.row];
      ...
      NSString * valid_end = [dicOne objectForKey:@"valid_end"];
        ...
        MissTaskDetailsViewController * showDetail = [[MissTaskDetailsViewController alloc] init];
...
-(void)getSelectMyJobLineLoseInfoByPage{
    ...
    [diction setValue:@"0" forKey:@"is_miss_pass"];
