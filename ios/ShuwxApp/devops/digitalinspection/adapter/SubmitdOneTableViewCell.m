...
#import "SubmitdOneTableViewCell.h"

@implementation SubmitdOneTableViewCell
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary * dicOne  =   [self.dataSource objectAtIndex:position];
       NSString * pointOption =   [dicOne objectForKey:@"pointOption"];
    int pointSize = pointOption.length ;
    
    int pointWidth = pointSize * 10 + 50;

    return CGSizeMake(pointWidth, 40);
...
- ( UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    int curPositionInt = -1;
   if ([[hitPoint.myValue allKeys] containsObject:self.curPosition]) {
               NSMutableDictionary * curDic =  [hitPoint.myValue objectForKey:self.curPosition];
             NSString * curPosition =  [curDic objectForKey:@"position"];
             curPositionInt = [curPosition intValue];
         }

         if (curPositionInt == position) {
             cell.xuanxView.backgroundColor = [UIColor colorWithRed:0/255.0 green:137/255.0 blue:225/255.0 alpha:1];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SubOneCollectionViewCell *cell = (SubOneCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
      ...
      int position =  indexPath.row + indexPath.section;
    NSString *string = [NSString stringWithFormat:@"%d",position];
  
    NSDictionary * dicOne  =   [self.dataSource objectAtIndex:position];
      NSString * pointOption =   [dicOne objectForKey:@"pointOption"];
    
     NSMutableDictionary * submitDiction = [NSMutableDictionary dictionary];
     [submitDiction setValue:pointOption forKey:@"pointOption"];
    [submitDiction setValue:string forKey:@"position"];
    
      HitPointViewController * hitPoint = (HitPointViewController *)self.myViewController;
     [hitPoint.myValue setObject:submitDiction forKey:self.curPosition];
