...
#import "MainAllOrderViewController.h"
...
@interface MainAllOrderViewController ()

@end

@implementation MainAllOrderViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"MainAllOrderViewController"];

}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:@"MainAllOrderViewController"];
}
...
- (void)initView{
    ...
    [QTSub(self, MyEventBus) next:^(MyEventBus *event) {
           ...
           if ([weakSelf getNSStringEqual:@"MainAllOrderViewController":myErrorId]) {
                   [weakSelf clearData];
                        [weakSelf getSelectWaitOrderPageList];
           }
          }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
     ...
     NSDictionary * dicOne  =   [self.dataSource objectAtIndex:indexPath.row];
        ...
        int receiveTimeOutInt = 0;
       if( [self getKeyIsNoValueIsNull:dicOne:@"receiveTimeOut"]){
            receiveTimeOutInt =  [receiveTimeOut intValue];
        }
        ...
        if ([self getKeyIsNoValueIsNull:dicOne:@"start_set"]) {
            start_set_int = [start_set intValue];
        }
        if (start_set_int == 2) {
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 1){
     ...
     [self getSelectWaitOrderPageList];
...
-(void)getSelectWaitOrderPageList{
    ...
    NSString * url = [self getPinjieNSString:baseUrl :selectWaitOrderPageList];
