...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"MyOrderIngViewController"];
 //   [self getSelectMyJobLineInfoByPage];
}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:@"MyOrderIngViewController"];
}
...
-(void)setScrollView{
    UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, 414, 60)];
    scrollView.showsHorizontalScrollIndicator = FALSE;//水平滚动条
    int myLength = 500;
    
    [self.view addSubview:scrollView];
    
    self.view1 = [[UIView alloc] initWithFrame:CGRectMake(0,0,myLength,60)];
    self.view1.backgroundColor = [UIColor whiteColor];
    [scrollView addSubview:self.view1];
    scrollView.contentSize = CGSizeMake(myLength, 60);
    int myLengthtwo = 10;
    ...
    for (int i = 0;i<5;i++) {
        ...
        sinaNme.frame = CGRectMake(myLengthtwo, 15, 50, 30);
        if (i == 1) {
            [sinaNme setTitle:@"待处理"  forState:(UIControlStateNormal)];
            sinaNme.frame = CGRectMake(myLengthtwo, 15, 50, 30);
        }
        if (i == 2) {
            [sinaNme setTitle:@"申请处理"  forState:(UIControlStateNormal)];
            sinaNme.frame = CGRectMake(myLengthtwo, 15, 60, 30);
        }
        if (i ==3) {
            [sinaNme setTitle:@"处理审核通过"  forState:(UIControlStateNormal)];
            sinaNme.frame = CGRectMake(myLengthtwo, 15, 80, 30);
        }
        if (i ==4) {
            [sinaNme setTitle:@"处理中"  forState:(UIControlStateNormal)];
            sinaNme.frame = CGRectMake(myLengthtwo, 15, 50, 30);
        }
    ...
    for (UIView *subView in self.view1.subviews) {
        if ( [subView isKindOfClass:[UIButton class]]) {
            ...
            if ([self getNSStringEqual:@"全部":myTitle]) {
...
-(void)setOrderSelect:(NSString *)curString{
    ...
    if ([self getNSStringEqual:curString:@"待处理"]) {
        status = 2;
    }
    if ([self getNSStringEqual:curString:@"申请处理"]) {
        status = 3;
    }
    if ([self getNSStringEqual:curString:@"处理审核通过"]) {
        status = 4;
    }
    if ([self getNSStringEqual:curString:@"处理中"]) {
        status = 5;
    }
    [self clearData];
...
-(void)viewDidLayoutSubviews{
    [super viewDidLayoutSubviews];
       self.uitableView.frame = CGRectMake(0, 60, SCREEN_WIDTH, SCREEN_HEIGHT - NAV_HEIGHT -44 - 60);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        start_set  =  [self getIntegerValue:start_set];
        ...
        int start_setInt = 0;
        if (![self isBlankString:start_set]) {
...
-(void)getSelectReceiveOrderInfoByPage{
    ...
    [diction setValue:@(20) forKey:@"pageSize"];
      ...
      [diction setValue:@(2) forKey:@"table_value"];
     
    if (status != 0) {
        [diction setValue:@(status) forKey:@"status"];
    }
 
    NSString * url = [self getPinjieNSString:baseUrl :selectMyMasOrderInfoPageList];
