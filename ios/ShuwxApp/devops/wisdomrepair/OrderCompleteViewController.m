...
@interface OrderCompleteViewController ()<ChartViewDelegate,PGDatePickerDelegate>
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"OrderCompleteViewController"];
  
}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:@"OrderCompleteViewController"];
}
-(void)setScan{
...
-(void)getOrderEndRateView{
    ...
    NSString * url = [self getPinjieNSString:baseUrl :orderEndRateView];
...
-(void)setOrderEndRateView:(NSMutableDictionary *) nsmutable{
  ...
  totalString =  [self getPinjieNSString:@"   ":totalString];
    ...
    self.noStartList = [nsmutable objectForKey:@"noStartList"];
      self.vingList = [nsmutable objectForKey:@"vingList"];
      self.vendList = [nsmutable objectForKey:@"vendList"];
    NSMutableArray * timeList = [nsmutable objectForKey:@"timeList"];
    
     NSString * curDays = [self getCurrentDays];
    int curDaysInt = [curDays intValue] - 1;
    [self setDayInfor:curDaysInt];
...
-(void)setDayInfor:(int) myInt{
   ...
   int v_end_count = [[self.vendList objectAtIndex:myInt]intValue];
    ...
    NSString *vEnt = [NSString stringWithFormat:@"%d", v_end_count];
       ...
       [self.waitButtonBottom setTitle:noStart  forState:(UIControlStateNormal)];
         [self.ingButtonBottom setTitle:vInt  forState:(UIControlStateNormal)];
         [self.endButtonBottom setTitle:vEnt  forState:(UIControlStateNormal)];
    
     NSString * singleDayTotal = [NSString stringWithFormat:@"%d", myInt + 1];
      NSString *allDaySum = [NSString stringWithFormat:@"%d", dayAll];
    singleDayTotal = [self getPinjieNSString:singleDayTotal:@"日报修总数"];
       allDaySum = [self getPinjieNSString:singleDayTotal:allDaySum];
       allDaySum = [self getPinjieNSString:singleDayTotal:@"单"];
    self.dayTotalValue.text = singleDayTotal;
