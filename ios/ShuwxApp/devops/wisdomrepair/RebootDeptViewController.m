...
#import "RebootDeptViewController.h"
...
@interface RebootDeptViewController ()<TreeTableCellThreeDelegate>

@end

@implementation RebootDeptViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"转交新员工";
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"RebootDeptViewController"];
}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:@"RebootDeptViewController"];
}
...
-(void)getSelectTurnManTree{
    ...
    NSString * url = [self getPinjieNSString:baseUrl :selectRebootManTree];
...
//            int select_id = [[itemDic objectForKey:@"select_id"] intValue];
//            NSString * proName = [itemDic objectForKey:@"user_name"];
//            Node *province1 = [[Node alloc] initWithParentId:-1 nodeId:select_id name:proName depth:depthInt expand:YES:2:0:@"":@""];
...
-(void)setToDialog:(NSString *)nameStr :(int)nodeId{
    ...
    NSString * titleStr = [self getPinjieNSString:@"确定转交给":nameStr] ;
    ...
    CKAlertAction *sure = [CKAlertAction actionWithTitle:@"确定" handler:^(CKAlertAction *action) {
        ...
        [self setRebootDept:nameStr:nodeId];
...
-(void)setRebootDept:(NSString *)nameStr :(int)nodeId{
    
    NSMutableDictionary * mydiction = [NSMutableDictionary dictionary];
    [mydiction setValue:@(nodeId) forKey:@"id"];
    [mydiction setValue:nameStr forKey:@"name"];
    
    MyEventBus * myEvent = [[MyEventBus alloc] init];
          myEvent.errorId = @"RebootOrderViewController";
