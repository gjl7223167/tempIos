...
- (BOOL)writeCacheData:(NSCachedURLResponse *)cachedURLResponse withRequest:(NSURLRequest *)request {
    ...
    double ddggg =  [date timeIntervalSince1970];
    NSString * timeStr = [NSString stringWithFormat:@"%f",ddggg];
    NSString * ggsg = cachedURLResponse.response.MIMEType;
//    NSString * ggaggeee = cachedURLResponse.response.textEncodingName;
    NSString * ggaggeee = @"utf-8";
    NSDictionary *info = @{@"time" : timeStr,
                           @"MIMEType" : ggsg,
                           @"textEncodingName" :ggaggeee };
