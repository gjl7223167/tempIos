//
//  QualityInspectionPlanManageViewController.m
//  ShuwxApp
//
//  Created by XuWei de apple on 2021/3/11.
...
@interface QualityInspectionPlanManageViewController ()<SGPageTitleViewDelegate, SGPageContentScrollViewDelegate>
...
@implementation QualityInspectionPlanManageViewController
...
-(void)setNavi
{
    self.navigationItem.title = @"质检计划";
...
- (void)openScanVCWithStyle:(LBXScanViewStyle*)style
{
    ...
    vc.scanType = scanQualityInspectionPlanManage;
...
-(void)sousuoClick
{
    ...
    vc.searchFrom = searchQualityInspectionPlanManage;
    NSDictionary *dic = self.titleArr[self.pageTitleView.selectedIndex];
    vc.paramStr = [NSString stringWithFormat:@"%@",dic[@"id"]];
    vc.paramSubStr = dic[@"dic_code"];
    QualityModelMainViewController *modelVc = self.childArr[0];
    vc.propertyArr = modelVc.propertyArr;
...
-(void)getTitleData
{
    ...
    @"QOM_Inspection_status",@"dic_type",
    nil];
...
- (void)setupMain{
    ...
    for (NSDictionary *dic in self.titleArr) {
        ...
        QualityModelMainViewController *oneView = [[QualityModelMainViewController alloc] init];
