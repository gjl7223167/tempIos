//
//  SupplierChooseViewController.m
//  ShuwxApp
//
...
@interface SupplierChooseViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation SupplierChooseViewController
...
-(void)sousuoClick
{
    ...
    vc.searchFrom = searchSupplierChoose;//SupplierChoose
    vc.SearchBackB = ^(NSDictionary *  _Nullable dic) {
        
        if (self.SupplierBack) {
            self.SupplierBack(dic);
        }
        [self.navigationController popViewControllerAnimated:YES];
    };
...
-(void)getSupplierChooseData
{
    
    [UrlRequest requestSupplierListDataWithParam:nil completion:^(BOOL result, id  _Nonnull data, NSString * _Nonnull error) {
        if (result) {
...
-(void)getRecommendSupplierData
{
    ...
    NSDictionary * param = [NSDictionary dictionaryWithObjectsAndKeys:
                            ...
                            flag,@"flag",
                            nil];
    [UrlRequest requestRecommendSupplierWithParam:param completion:^(BOOL result, id  _Nonnull data, NSString * _Nonnull error) {
           if (result) {
               [self.recommendDataArr addObjectsFromArray:data];
           }
        
        [self getSupplierChooseData];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        infoStr = [NSString stringWithFormat:@"常用推荐"];
    }
