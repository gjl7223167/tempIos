...
//    if (seconds < 3) {
//        if (compressComplete) {
//            compressComplete(NO, nil,0,seconds);
//        }
//        return;
//    }
    //压缩前原视频大小MB
...
//    NSInteger frameRate = [videoTrack nominalFrameRate];
    ...
    if (kbps <= (compressBiteRate / 1024)) {
        ...
        if (compressComplete) {
            compressComplete(NO, videoUrl,fileSizeMB,seconds);
        }
        return;
    }
    //指定压缩视频沙盒根目录
        ...
        switch (writer.status) {
            case AVAssetWriterStatusWriting:
            {
                ...
                [writer finishWritingWithCompletionHandler:^{
                    if (compressComplete) {
                        compressComplete(YES, outPutPathURL,fileSizeMB,seconds);
                    }
                }];
            }
                break;
