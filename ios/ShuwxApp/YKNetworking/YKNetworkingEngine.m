...
- (void)requestMethod:(YKRequestMethodType)method
    ...
    switch (method) {
        case YKRequestMethodTypePOST:{
            dataTask = [self POST:url parameters:parameters progress:progress success:success failure:failure];
        }
            break;
        case YKRequestMethodTypePUT:{
            dataTask = [self PUT:url parameters:parameters success:success failure:failure];
        }
            break;
        case YKRequestMethodTypePATCH:{
            dataTask = [self PATCH:url parameters:parameters  success:success failure:failure];
        }
            break;
        case YKRequestMethodTypeDELETE:{
            dataTask = [self DELETE:url parameters:parameters  success:success failure:failure];
        }
            break;
        default:{
            dataTask = [self GET:url parameters:parameters  progress:progress success:success failure:failure];
        }
            break;
    }
}
...
- (NSURLSessionDataTask *)uploadFileWithURL:(NSString *)url
                                 ...
                                 parameters:(nullable NSDictionary *)parameters
                                   ...
                                   progress:(nullable YKHttpRequestProgressBlock)progress
                                    success:(nullable YKHttpRequsetSuccessBlock)success
                                    failure:(nullable YKHttpRequsetFailBlock)failure{
    NSURLSessionDataTask *uploadTask = [self  POST:[self stringUTF8Encoding:url] parameters:parameters  constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        NSError *error = nil;
