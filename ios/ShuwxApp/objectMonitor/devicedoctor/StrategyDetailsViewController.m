...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"StrategyDetailsViewController"];
    // [self getSelectObjectUnderList];
}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:@"StrategyDetailsViewController"];
}
-(void)setScan{
...
- (void)initView{
    ...
    [self.strateText setEditable:NO];
    self.strateText.text =  self.strateDescri;
}
-(void)setPeizhiBtn{
    ...
    for(NSString *key in keyArr) {
        ...
        if ([self isNullDictionary:dicOne]) {
            return;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        if ([self isBlankString:myValue]) {
            myValue = @"";
        }
        
        cell.sdValue.text = myValue;
        [self.myValue setObject:myValue forKey:string];
...
-(void)getSelectObjectUnderByUnderId{
    ...
    [diction setValue:self.selectId forKey:@"rule_id"];
    
    NSString * url = [self getPinjieNSString:baseUrl :selectObjectUnderListByRuleId];
...
-(void)setSelectObjectUnderByUnderId:(NSMutableDictionary *) nsmutable{
    NSMutableArray * mutableArray = [nsmutable objectForKey:@"valueList"];
    for (NSMutableDictionary * nsmuTab in mutableArray) {
